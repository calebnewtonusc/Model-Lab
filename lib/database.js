...
try {
  ...
  // Check if runs table has model_type column
  const runsModelTypeInfo = db.prepare("PRAGMA table_info(runs)").all();
  const runsHasModelType = runsModelTypeInfo.some(col => col.name === 'model_type');

  if (!runsHasModelType) {
    db.exec('ALTER TABLE runs ADD COLUMN model_type TEXT');
    console.log('✓ Added model_type column to runs table');
  }
} catch (error) {
...
const createRun = db.prepare(`
  INSERT INTO runs (id, name, description, dataset_id, model_type, status, config, metrics, seed, commit_hash, project_id, created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
module.exports = {
  ...
  createRun: (run) => {
    ...
    createRun.run(
      ...
      run.model_type || run.modelType || null,
      run.status || 'running',
  ...
  // Serialization helpers (for testing and external use)
  serializeRun: (data) => {
    const result = { ...data };
    const jsonFields = ['array', 'object', 'hyperparameters', 'metrics', 'config', 'tags', 'metadata', 'schema'];
  ...
  deserializeRun: (data) => {
    const result = { ...data };
    const jsonFields = ['hyperparameters', 'metrics', 'config', 'tags', 'metadata', 'schema', 'slices', 'failure_examples', 'files'];
    for (const field of jsonFields) {
